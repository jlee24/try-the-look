...
def save_origin_grabcut(img, bbox):
    ...
    if aspect_ratio < 1:  # vertical
        ...
        # img = cv2.resize(img, tuple(new_d ims))
...
def get_user_selection(img):
    ...
    # Return the selected rectxangle
...
# Grabcut loop
# This function contains the actual implementation of the entire grabcut_code
# algorithm
# 
# img - image to perform segmentation on
# image_name - image name for logging purposes
# user_interaction - boolean specifing if user interaction should be enabled
# num_iterations - number of iterations to run grabcut_code for
...
def yolo(img):
    ...
    grabcut(img, bbox, None, num_iterations=5, debug=True, drawImage=True)
